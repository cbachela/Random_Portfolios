  ...
  ############################################################################
  ### RANDOM PORTFOLIOS - MAX RANDOM CENTROID SAMPLING - FACTOR CONTROL - USA
  ############################################################################
  ...
  # BT_MF$spec$cons_fun <- c("box", "sector", "sectorLB", "vola") # Notice that we approximate the quadratic variance constraint with a linear version.
  # BT_MF$spec$vola_multiple <- 1.1
  # BT_MF$data <- BT_Fctrl$data
  
  
  
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS - NO CONTROL
  # --------------------------------------------------------------------------
  
  BT1 <- BT_MF$copy()
  BT1$spec$keep_all_rebdates <- TRUE
  BT1$spec$portfolio <- "maxRC"
  BT1$spec$name <- "maxRC_multifac_usa"
  BT1$spec$n_sim <- 10^2 * 5
  BT1$output <- list()
  # debugonce( BT1$maxRCPortfolio )
  BT1$runLoop()
  ...
  colnames(sim_1) <- paste0("rp_maxRC_uniform", 1:ncol(sim_1))
  BT1$output$simulations <- sim_1

  # Save
  BT1$save( wd = paste0(wd_data, "waRehouse/"), name = BT1$spec$name, without_data = TRUE )
  
  
  
  # # Bias to more concentrated portfolios
  # BT1a <- BT_MF$copy()
  # BT1a$spec$keep_all_rebdates <- TRUE
  # BT1a$spec$portfolio <- "maxRC"
  # BT1a$spec$name <- "maxRC_multifac_usa_npos10"
  # BT1a$spec$n_sim <- 10^2 * 5
  # BT1a$spec$n_pos <- 10
  # BT1a$output <- list()
  # # debugonce( BT1a$maxRCPortfolio )
  # BT1a$runLoop()
  # 
  # # Simulate
  # tmp <- lapply( BT1a$output, FUN = simFUN )
  # sim_1a <- do.call( cbind, tmp )
  # sim_1a <- sim_1a[isWeekday(time(sim_1a)), ]
  # colnames(sim_1a) <- paste0("rp_maxRC_uniform", 1:ncol(sim_1), "a")
  # BT1a$output$simulations <- sim_1a
  # 
  # # Save
  # BT1a$save( wd = paste0(wd_data, "waRehouse/"), name = BT1a$spec$name, without_data = TRUE )
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH VALUE CONTROL
  # --------------------------------------------------------------------------
  ...
  BT2$spec$portfolio <- "maxRC"
  BT2$spec$n_sim <- 10^2 * 5
  BT2$spec$name <- "maxRC_multifac_usa_valctrl"
  BT2$output <- list()
  BT2$runLoop()
  
  # today <- "2002-03-06"
  # RBE <- rebalenv( rebdate = today )
  # debugonce( BT2$maxRCPortfolio )
  # BT2$maxRCPortfolio( today )
  
  # Simulate
  tmp <- lapply( BT2$output[ grepl("maxRC", names(BT2$output)) ], FUN = simFUN )
  sim_2 <- do.call( cbind, tmp )
  sim_2 <- sim_2[isWeekday(time(sim_2)), ]
  colnames(sim_2) <- paste0("rp_maxRC_uniform_valctrl", 1:ncol(sim_2))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH QUALITY CONTROL
  # --------------------------------------------------------------------------
  ...
  BT3$spec$portfolio <- "maxRC"
  BT3$spec$n_sim <- 10^2 * 5
  BT3$spec$name <- "maxRC_multifac_usa_qualctrl"
  BT3$output <- list()
  BT3$runLoop()
  
  # Simulate
  tmp <- lapply( BT3$output[ grepl("maxRC", names(BT3$output)) ], FUN = simFUN )
  sim_3 <- do.call( cbind, tmp )
  sim_3 <- sim_3[isWeekday(time(sim_3)), ]
  colnames(sim_3) <- paste0("rp_maxRC_uniform_qualctrl", 1:ncol(sim_3))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH SIZE CONTROL
  # --------------------------------------------------------------------------
  
  BT4 <- BT_Fctrl$copy()
  BT4$spec$factor_names <- "size"
  BT4$spec$factor_names_constraints <- BT4$spec$factor_names
  BT4$spec$keep_all_rebdates <- TRUE
  BT4$spec$portfolio <- "maxRC"
  BT4$spec$n_sim <- 10^2 * 5
  BT4$spec$name <- "maxRC_multifac_usa_sizectrl"
  BT4$output <- list()
  BT4$runLoop()
  
  # Simulate
  tmp <- lapply( BT4$output[ grepl("maxRC", names(BT4$output)) ], FUN = simFUN )
  sim_4 <- do.call( cbind, tmp )
  sim_4 <- sim_4[isWeekday(time(sim_4)), ]
  colnames(sim_4) <- paste0("rp_maxRC_uniform_sizectrl", 1:ncol(sim_4))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH SIZE STANDARDIZED CONTROL
  # --------------------------------------------------------------------------
  
  BT5 <- BT_Fctrl$copy()
  BT5$spec$factor_names <- "size_stdz"
  BT5$spec$factor_names_constraints <- BT5$spec$factor_names
  BT5$spec$keep_all_rebdates <- TRUE
  BT5$spec$portfolio <- "maxRC"
  BT5$spec$n_sim <- 10^2 * 5
  BT5$spec$name <- "maxRC_multifac_usa_sizestdzctrl"
  BT5$output <- list()
  BT5$runLoop()
  
  # Simulate
  tmp <- lapply( BT5$output[ grepl("maxRC", names(BT5$output)) ], FUN = simFUN )
  sim_5 <- do.call( cbind, tmp )
  sim_5 <- sim_5[isWeekday(time(sim_5)), ]
  colnames(sim_5) <- paste0("rp_maxRC_uniform_sizestdzctrl", 1:ncol(sim_5))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH VALUE, QUALITY, SIZE STANDARDIZED CONTROL
  # --------------------------------------------------------------------------
  
  BT6 <- BT_Fctrl$copy()
  BT6$spec$factor_names <- c("value", "quality", "size_stdz")
  BT6$spec$factor_names_constraints <- BT6$spec$factor_names
  BT6$spec$keep_all_rebdates <- TRUE
  BT6$spec$portfolio <- "maxRC"
  BT6$spec$n_sim <- 10^2 * 5
  BT6$spec$name <- "maxRC_multifac_usa_valqualsizectrl"
  BT6$output <- list()
  BT6$runLoop()
  
  # Simulate
  tmp <- lapply( BT6$output[ grepl("maxRC", names(BT6$output)) ], FUN = simFUN )
  sim_6 <- do.call( cbind, tmp )
  sim_6 <- sim_6[isWeekday(time(sim_6)), ]
  colnames(sim_6) <- paste0("rp_maxRC_uniform_valqualsizectrl", 1:ncol(sim_6))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH MOMENTUM CONTROL
  # --------------------------------------------------------------------------
  
  BT7 <- BT_Fctrl$copy()
  BT7$spec$factor_names <- c("momentum_stdz")
  BT7$spec$factor_names_constraints <- BT7$spec$factor_names
  BT7$spec$keep_all_rebdates <- TRUE
  BT7$spec$portfolio <- "maxRC"
  BT7$spec$n_sim <- 10^2 * 5
  BT7$spec$name <- "maxRC_multifac_usa_momctrl"
  BT7$output <- list()
  BT7$runLoop()
  
  # Simulate
  tmp <- lapply( BT7$output[ grepl("maxRC", names(BT7$output)) ], FUN = simFUN )
  sim_7 <- do.call( cbind, tmp )
  sim_7 <- sim_7[isWeekday(time(sim_7)), ]
  colnames(sim_7) <- paste0("rp_maxRC_uniform_momctrl", 1:ncol(sim_7))
  ...
  # --------------------------------------------------------------------------
  # BACKTEST RP'S WITHIN CONSTRAINTS WITH 
  ...
  BT8$spec$keep_all_rebdates <- TRUE
  BT8$spec$portfolio <- "maxRC"
  BT8$spec$n_sim <- 10^2 * 5
  BT8$spec$name <- "maxRC_multifac_usa_valqualsizemomctrl"
  BT8$output <- list()
  BT8$runLoop()

  # Simulate
  tmp <- lapply( BT8$output[ grepl("maxRC", names(BT8$output)) ], FUN = simFUN )
  sim_8 <- do.call( cbind, tmp )
  sim_8 <- sim_8[isWeekday(time(sim_8)), ]
  colnames(sim_8) <- paste0("rp_maxRC_uniform_valqualsizemomctrl", 1:ncol(sim_8))
